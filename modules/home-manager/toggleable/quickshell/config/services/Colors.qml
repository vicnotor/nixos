...
Singleton {
  ...
  readonly property list<string> colorNames: ["rosewater", "flamingo", "pink", "mauve", "red", "maroon", "peach", "yellow", "green", "teal", "sky", "sapphire", "blue", "lavender"]
  ...
  function load(data: string, isPreview: bool): void {
    ...
    for (const [name, color] of Object.entries(scheme.colors)) {
      const propName = colorNames.includes(name) ? name : `m3${name}`;
  ...
  component M3Palette: QtObject {
    ...
    property color m3background: "#0F0F0F"
    property color m3onBackground: "#E5E1E9"
    property color m3surface: "#0F0F0F"
    property color m3surfaceDim: "#0F0F0F"
